...
const (
	getByStatementForVS = `SELECT d.id, d.created_at, d.updated_at, d.validator_id, d.amount, d.eth_block_height, d.statistics_type FROM validator_statistics d WHERE d.statistics_type = $1 `
	...
	calculateTotalStake = `INSERT INTO validator_statistics (updated_at, validator_id, amount, eth_block_height, statistics_type) 
 								SELECT NOW(), t1.validator_id, sum(t1.amount) AS amount, $1 AS eth_block_height, $2 AS statistics_type FROM
	...
	calculateActiveNodes = `INSERT INTO validator_statistics (updated_at, validator_id, amount, eth_block_height, statistics_type) 
									(SELECT NOW(), validator_id, count(*) AS amount, $1 AS eth_block_height, $2 AS statistics_type FROM nodes
									WHERE validator_id = $3 AND status = $4 GROUP BY validator_id, skale_id ORDER BY eth_block_height DESC)`
	calculateLinkedNodes = `INSERT INTO validator_statistics (updated_at, validator_id, amount, eth_block_height, statistics_type) 
									(SELECT NOW(), validator_id, count(*) AS amount, $1 AS eth_block_height, $2 AS statistics_type FROM nodes
...
func (d *Driver) CalculateTotalStake(ctx context.Context, params structs.QueryParams) error {
	_, err := d.db.Exec(calculateTotalStake, params.ETHBlockHeight, structs.TotalStakeStatisticsTypeVS, params.ValidatorId, params.ETHBlockHeight, structs.Accepted, structs.UndelegationRequested)
	if err == nil {
		_, err = d.db.Exec(updateTotalStake, params.ValidatorId, structs.TotalStakeStatisticsTypeVS, params.ValidatorId, params.ETHBlockHeight)
	}
	return err
}
...
func (d *Driver) CalculateActiveNodes(ctx context.Context, params structs.QueryParams) error {
	_, err := d.db.Exec(calculateActiveNodes, params.ETHBlockHeight, structs.ActiveNodesStatisticsTypeVS, params.ValidatorId, structs.Active)
	if err == nil {
		_, err = d.db.Exec(updateActiveNodes, params.ValidatorId, structs.ActiveNodesStatisticsTypeVS, params.ValidatorId, params.ETHBlockHeight)
	}
	return err
}
...
func (d *Driver) CalculateLinkedNodes(ctx context.Context, params structs.QueryParams) error {
	_, err := d.db.Exec(calculateLinkedNodes, params.ETHBlockHeight, structs.LinkedNodesStatisticsTypeVS, params.ValidatorId)
	if err == nil {
		_, err = d.db.Exec(updateLinkedNodes, params.ValidatorId, structs.LinkedNodesStatisticsTypeVS, params.ValidatorId, params.ETHBlockHeight)
	}
	return err
}
