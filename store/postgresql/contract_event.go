...
const (
	...
	getByStatementCE         = `SELECT e.id, e.created_at, e.updated_at, e.contract_name, e.event_name, e.block_height, e.time, e.params, e.removed, e.bound_type, e.bound_id FROM contract_events e `
...
func (d *Driver) GetContractEvents(ctx context.Context, params structs.QueryParams) (contractEvents []structs.ContractEvent, err error) {
	...
	for rows.Next() {
		...
		//err = rows.Scan(&e.ID, &e.CreatedAt, &e.UpdatedAt, &e.ContractName, &e.EventName, pq.Array(&e.ContractAddress), &e.BlockHeight, &e.Time, pq.Array(&e.TransactionHash), &e.Params, &e.Removed, &e.BoundType, pq.Array(&e.BoundId), pq.Array(&e.BoundAddress))
		// TODO: find a solution to read byte array properly for contract_address, transaction_hash, bound_address
		err = rows.Scan(&e.ID, &e.CreatedAt, &e.UpdatedAt, &e.ContractName, &e.EventName, &e.BlockHeight, &e.Time, &e.Params, &e.Removed, &e.BoundType, pq.Array(&e.BoundId))
		if err != nil {
			return nil, err
		}
